...
export function TeamCard({ member }: { member: TeamMember }) {
  return (
    <motion.div
      whileHover={{ y: -6, boxShadow: "0 20px 40px -12px rgba(43, 87, 232, 0.25)" }}
    ...
    >
      {member.photoUrl ? (
        ...
        <div className="flex h-20 w-20 items-center justify-center rounded-full bg-linear-to-br from-primary to-primary-dark text-2xl font-semibold text-white">
      ...
      <div className="mt-5 flex gap-3 text-sm font-semibold">
        <a href={member.linkedinUrl} target="_blank" rel="noreferrer" className="text-primary-dark hover:underline">
          LinkedIn
        </a>
        <a href={member.githubUrl} target="_blank" rel="noreferrer" className="text-primary-dark hover:underline">
          GitHub
        </a>
      </div>
    </motion.div>
