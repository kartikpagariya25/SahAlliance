...
import { monToWei, formatEtherNumber } from "../lib/format";
import { inrEquivalent } from "../lib/currency";
...
export function RepayModal({
  ...
  const remaining = amount - amountRepaid;
  const remainingMon = formatEtherNumber(remaining);
  ...
  const effectiveAmount = useCustom ? Number(customInput || 0) : remainingMon;
  const valid = effectiveAmount > 0 && effectiveAmount <= remainingMon + 1e-9;
  const isFullRepayment = !useCustom || effectiveAmount >= remainingMon;
  ...
  async function handleSubmit() {
    ...
    try {
      const receipt = await runWrite(() =>
        client.connect(persona.address).repayLoan(loanId, { value: monToWei(String(effectiveAmount)) })
      );
          ...
          {!done ? (
            <motion.div key="form" exit={{ opacity: 0 }}>
              <div className="flex items-center justify-between">
                <h2 className="font-display text-xl text-ink">Repay your loan</h2>
                <button onClick={onClose} aria-label="Close" className="text-ink-soft hover:text-ink">
                  ✕
                </button>
              </div>
              <p className="mt-1 text-sm text-ink-soft">
                {inrEquivalent(formatEtherNumber(amountRepaid))} repaid so far of {inrEquivalent(formatEtherNumber(amount))}
              </p>

              <div className="mt-4 h-2 overflow-hidden rounded-full bg-frost-soft">
                <div
                  className="h-full rounded-full bg-success transition-all duration-500"
                  style={{ width: `${(Number(amountRepaid) / Number(amount)) * 100}%` }}
                />
              </div>
              ...
              >
                <span>
                  ...
                  <span className="block text-xs text-ink-soft">Settles this loan completely</span>
                </span>
                <span className="font-display text-lg">{inrEquivalent(remainingMon)}</span>
              </button>
              ...
              >
                {useCustom ? "Pay the full remaining amount instead" : "Pay a partial amount instead"}
              </button>

              {useCustom && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-3">
                  <label className="block text-sm font-medium text-ink-soft">
                    Amount (MON) — up to {remainingMon}
                    <input
                      type="number"
                      step="0.001"
                      min="0"
              ...
              >
                {pending ? (
                  ...
                  `Repay ${inrEquivalent(effectiveAmount)}${isFullRepayment ? " — settle in full" : ""}`
                )}
              </button>
            </motion.div>
          ) : (
            ...
            >
              ...
              <p className="mt-4 font-display text-lg text-ink">
                {isFullRepayment ? "Loan fully repaid" : "Repayment confirmed"}
              </p>
